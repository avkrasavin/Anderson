    ...
    def matrix(self, m_d, m_c, m_b, n_down, n_up, n_max):
                ...
                for p in range(m_c + m_d, 2 * m_c + m_d):
                    ...
                    if coef != 0:
                        for T in range(0, R_bose * R_up * R_down, R_bose * R_down):
                            ...
                            data[s0: s0 + R_bose] = coef * V_cd[j - (2 * m_c + m_d), p - (m_c + m_d)] * self.sign(p, j,
                                                                                                                  basis[
                                                                                                                      i])
                            s0 += R_bose
                            # перескок с кластера на ванну
                    ...
                    if coef != 0:
                        for T in range(0, R_bose * R_up * R_down, R_bose * R_down):
                            line[s0: s0 + R_bose] = range(i + T, i + T + R_bose)
                            col[s0: s0 + R_bose] = range(index + T, index + T + R_bose)
                            data[s0: s0 + R_bose] = coef * V_cd[p - (m_c + m_d), j - (2 * m_c + m_d)] * self.sign(p, j,
                                                                                                                  basis[
                                                                                                                      i])
                            s0 += R_bose
        bar = progressbar.ProgressBar()
        print('Complete jumps from cluster to fermi bath')
        ...
        for i in bar(range(0, R_bose * R_down, R_bose)):
            ...
            for j in range(2 * m_c + m_d, 2 * (m_c + m_d)):
                ...
                iks += 1
        bar = progressbar.ProgressBar()
        print('Complete jumps on the cluster')
        # бозоны
        s0 = np.where(data == 0)[0][0]
        s = 0
        ...
        for i in bar(range(0, R[0], R_bose)):
            ...
            data[s0:s0 + R_bose] = sum(
                self.e_c * basis[i, 0:m_c] + self.e_c * basis[i, (m_d + m_c):(m_d + 2 * m_c)]) + self.U_d * sum(
                basis[i, m_c:(m_d + m_c)] * basis[i, (m_d + 2 * m_c):2 * (m_d + m_c)])
            for j in neigbors_d_up.keys():
                for k in neigbors_d_up[j]:
                    data[s0 + i:s0 + i + R_bose] += self.V_d * (basis[i, j] + basis[i, j + m_c]) * (
                    basis[i, k] + basis[i, k + m_c])
            s0 += R_bose
        bar = progressbar.ProgressBar()
        for i in bar(range(R_bose)):
